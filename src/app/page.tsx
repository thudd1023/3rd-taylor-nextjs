    ...
    <div className="container relative pt-20 pb-28 md:pt-28 md:pb-36">
      <div className="max-w-5xl">
        ...
        <div className="mt-10 flex flex-wrap items-center gap-4">
          <a href="#engine" className="group inline-flex items-center gap-2 bg-ink text-cream px-7 py-4 rounded-full font-semibold shadow-bold hover:bg-accent transition-all hover:scale-[1.02]">
      ...
      <div className="hidden lg:block absolute right-8 top-32 w-64 rotate-6 bg-cream rounded-2xl shadow-soft p-5 border border-border animate-float-in-1 hover:rotate-0 transition-transform duration-300">
      ...
      <div className="hidden lg:block absolute right-32 bottom-12 w-56 -rotate-3 bg-ink text-cream rounded-2xl shadow-soft p-5 animate-float-in-2 hover:rotate-0 transition-transform duration-300">
  ...
  return (
    <section className="py-24 md:py-32 bg-background">
      <div className="container max-w-5xl">
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-x-10 gap-y-3">
          {pains.map((pain, i) => (
            <div key={i} className="flex gap-3 py-3 border-b border-border/60">
...
const Reframe = () => (
  <section className="py-24 md:py-32 bg-secondary">
    <div className="container max-w-4xl text-center">
      ...
      <div className="mt-16 bg-ink text-cream rounded-3xl p-10 md:p-14 text-left shadow-bold">
        ...
        <div className="mt-8 grid sm:grid-cols-2 gap-4 text-sm">
          {["A clear messaging arc", "Intentional sequencing across channels", "The right mix of paid, content, and nurture", "Someone to connect it all together"].map(item => (
            <div key={item} className="flex items-center gap-3 bg-cream/5 border border-cream/10 rounded-xl px-4 py-3">
              <Check className="h-4 w-4 text-warm shrink-0" /><span>{item}</span>
            </div>
          ))}
        </div>
        <p className="mt-8 text-cream/70">That's where most teams get stuck. And that's exactly what we fix.</p>
      </div>
    </div>
  </section>
);
      ...
      <div className="container">
        ...
        <div className="mt-16 grid md:grid-cols-2 gap-6">
          {tiles.map(({ icon: Icon, title, desc }, i) => (
            <div key={title} className="group relative bg-cream border border-border rounded-3xl p-8 hover:border-accent transition-all hover:-translate-y-1 shadow-soft">
...
const Method = () => {
  ...
  return (
    <section id="method" className="py-24 md:py-32 bg-ink text-cream relative overflow-hidden">
      ...
      <div className="absolute -bottom-40 -left-40 h-96 w-96 rounded-full bg-warm/20 blur-3xl" />
      <div className="container relative max-w-5xl">
        ...
        <div className="mt-16 space-y-px">
          {steps.map((step, i) => (
            <div key={step.letter} className="group grid grid-cols-[auto_1fr_auto] items-center gap-6 md:gap-10 py-8 border-t border-cream/15 hover:bg-cream/5 transition-colors px-2 -mx-2 rounded-lg">
      ...
      <div className="container max-w-5xl">
        ...
        <div className="mt-12 space-y-3">
          {outcomes.map((o, i) => {
            ...
            const Icon = o.icon;
            return (
              <button key={i} onClick={() => setOpen(isOpen ? -1 : i)} className={`w-full text-left bg-cream border-2 ${isOpen ? "border-accent" : "border-border"} rounded-2xl p-6 md:p-8 transition-all`}>
    ...
    <section id="packages" className="py-24 md:py-32 bg-secondary">
      <div className="container">
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-6 items-start">
          {pkgs.map((p) => (
            <div key={p.name} className={`relative rounded-3xl p-8 md:p-10 flex flex-col h-full ${p.featured ? "bg-ink text-cream md:scale-105 md:-mt-4 shadow-bold" : "bg-cream border border-border"}`}>
    ...
    <section id="results" className="py-24 md:py-32 bg-background">
      <div className="container">
        ...
        <div className="mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border rounded-3xl overflow-hidden border border-border">
          {stats.map((s, i) => (
            <div key={i} className="bg-cream p-8 md:p-10 hover:bg-secondary transition-colors">
              <div className="font-display text-6xl md:text-7xl font-medium text-accent leading-none">{s.value}</div>
      ...
      <div className="container max-w-3xl">
        ...
        <div className="mt-12 space-y-3">
          {faqs.map((f, i) => {
            const isOpen = open === i;
            return (
              <div key={i} className={`border-2 rounded-2xl transition-all ${isOpen ? "border-accent bg-cream" : "border-border bg-cream"}`}>
